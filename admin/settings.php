	...
	<div class="wpup-page-header">
		<div class="wpup-container wpup-flex-box">
			<div class="wpup-logo-wrapper wpup-headline">
				<a href="#">
					<img src="<?php echo esc_url(WPUP_PLUGIN_URI . '/assets/images/wordpress-utility-and-performance-plugin-thememantis-logo.png'); ?>" class="wpup-logo" alt="<?php echo esc_html__('WP Utility & Performance WordPress Plugin', 'wp-utility-and-performance'); ?>">
				</a>
			</div>
		</div>
	</div>
			...
			<div id="post-body" class="metabox-holder columns-2">
				...
				<div id="post-body-content">
					...
					<form action="options-general.php?page=<?php echo esc_attr($this->plugin_name); ?>" method="post">
						...
						?>
							<div class="postbox">
								<h3 class="hndle"><?php echo esc_html($postbox['h3']); ?></h3>
								<?php
								foreach ( $postbox['inside'] as $inside ) {
								?>
									<div class="inside">
										<div class="wpup-row">
											<div class="wpup-label"><label for="<?php echo esc_attr($inside['id']); ?>"><?php echo esc_html($inside['text']); ?></label></div>
											<div class="wpup-field">
												<input class="wpup-switch-checkbox" name="<?php echo esc_attr($inside['id']); ?>" type="checkbox" id="<?php echo esc_attr($inside['id']); ?>" value="yes" <?php checked('yes', $this->settings[ $inside['id'] ]); ?> />
												<label for="<?php echo esc_attr($inside['id']); ?>" class="wpup-switch-toggle button-status"></label>
											</div>
										</div>
									</div>
								<?php } ?>
